...
const Rapottka = () => {
    ...
    const { contextTka } = useContext(AuthContext);
    ...
    async function getBidangPengembangan() {
        const res = await link.get('Bidangpengembangan/' + contextTka?.rombelidtapel)
    ...
    async function getNilaiPengembangan() {
        const resSatu = await link.get(`Nilaipengembangan/${contextTka?.siswarombelid}/1`)
        setNilaiPengembanganSatu(resSatu.data)

        const resDua = await link.get(`Nilaipengembangan/${contextTka?.siswarombelid}/2`)
        setNilaiPengembanganDua(resDua.data)
    }

    async function getFotoPengembangan() {
        const resSatu = await link.get(`Fotopengembangan/${contextTka?.siswarombelid}/1`)
        setFotoPengembanganSatu(resSatu.data)

        const resDua = await link.get(`Fotopengembangan/${contextTka?.siswarombelid}/2`)
        setFotoPengembanganDua(resDua.data)
    }

    async function getAbsenFisik() {
        const resSatu = await link.get(`Absenfisik/${contextTka?.siswarombelid}/1`)
        setAbsenFisikSatu(resSatu.data[0])

        const resDua = await link.get(`Absenfisik/${contextTka?.siswarombelid}/2`)
        setAbsenFisikDua(resDua.data[0])
    }

    async function getCapaianNgaji() {
        const resSatu = await link.get(`Capaianummitahfidz/${contextTka?.siswarombelid}/1`)
        setCapaianNgajiSatu(resSatu.data[0])

        const resDua = await link.get(`Capaianummitahfidz/${contextTka?.siswarombelid}/2`)
        setCapaianNgajiDua(resDua.data[0])
    }

    async function getCapaian() {
        const res = await link.get(`Capaian/${contextTka?.rombelidtapel}/${contextTka?.kelastingkat}`)
        setCapaian(res.data)
    ...
    async function getNilaiCapaian() {
        const resSatu = await link.get(`Nilaicapaian/${contextTka?.siswarombelid}/1`)
        setNilaiCapaianSatu(resSatu.data)

        const resDua = await link.get(`Nilaicapaian/${contextTka?.siswarombelid}/2`)
                ...
                <View style={{ flexDirection: 'column', flex: 1 }}>
                    ...
                    <ScrollView>
                        <View style={{ paddingHorizontal: 20, paddingBottom: 20 }}>
                            <Text style={{ fontWeight: 'bold', marginBottom: 10 }}>Kelas : {contextTka?.kelasnamakelas}</Text>
                            {
